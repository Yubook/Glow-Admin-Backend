...
io.sockets.on("connection", function (socket) {
  ...
  // socket.on('setActive',async function(data){
  //     const url = server_url + "setActive";
  //     const config ={
  //         headers:{
  ...
  socket.on("sendMessage", async function (data) {
    ...
    if (parseInt(result1.data.success) == 1) {
      ...
      //data.receiver_id = result1.data.receiver_id;
      //const url2 = server_url + "sendUnreadMessageNotification";
      //const result2 = await axios.post(url2,data,config);
      //console.log(result2);
    }
  });
  ...
  socket.on("getLocation", async function (data) {
    const url = server_url + "getLocation";
    const config = {
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + data.token,
    ...
    if (parseInt(result.data.success) == 1) {
      result.data.token = data.token;
      io.sockets.emit("getOrderLocation", result.data);
    }
  });

  socket.on("updateLocation", async function (data) {
    const url = server_url + "updateLocation";
    const config = {
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + data.token,
    ...
    if (parseInt(result1.data.success) == 1) {
      result1.data.token = data.token;
      io.sockets.emit("setLocation", result1.data);
    }
  });
});
